...
#include "error_monitor/error_monitor.h"       // для логирования ошибок
...
void deadline_monitor_init(deadline_monitor_t* monitor, uint32_t deadline_us) {
    ...
    monitor->count = 0;
    monitor->max_time_us = 0;
    monitor->min_time_us = UINT32_MAX;
    monitor->total_time_us = 0;
}

void deadline_monitor_start(deadline_monitor_t* monitor) {
    //monitor->start_time = platform_timer_get_us(); to do: platform timer
    monitor->active = true;
}
...
void deadline_monitor_stop(deadline_monitor_t* monitor, const char* context) {
    ...
    //monitor->stop_time = platform_timer_get_us(); to do: platform timer
    monitor->active = false;
